...
class offline_calibration_app(wx.Frame):
    ...
    def __init__(self, config, movie, calinfo, iris_detector=None,  batch=False, output=None, overwrite=False):
        ...
        self.output_file_for_offline = output
    ...
    def run_calibration(self,event):
        ...
        if not self.batch_mode:
            ...
            np.savez(self.output_file_for_offline,
                fitting_param=dlg.fitting_param, 
                precision=dlg.results[0],
                accuracy=dlg.results[1],
                max_error=dlg.results[2],
                results_detail=dlg.results[3],
                area_of_interest = aoi)
            dlg.Destroy()
...
if __name__ == '__main__':
    ...
    if args.batch:
        ...
        # test whether Output file can be opened
        if os.path.isfile(args.output) and (not args.overwrite):
            print('Output file ({}) already exists. Use --overwrite option to overwrite existing file.'.format(args.output))
            sys.exit()
        elif os.path.splitext(args.output)[1].lower() != '.npz':
            print('Filename extension of the output ({}) must be ".npz".'.format(args.output))
            sys.exit()
        else:
            try:
                open(args.output, 'w')
            except:
                DlgShowerror('Cannot open {}.'.format(args.output))
                sys.exit()
